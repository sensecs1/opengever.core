...
The script assumes you got an instance running on port ``${instance:http-address}``, a GEVER client called ``admin1`` and an omelette with ``ftw.mail`` in it installed. It will then feed the mail from stdin to
